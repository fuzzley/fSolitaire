---
name: phaser-groups-and-containers
...
**Related skills:** ../phaser-sprites-and-images/SKILL.md, ../phaser-physics-arcade/SKILL.md
...
Physics groups extend Group with automatic body assignment. See ../phaser-physics-arcade/SKILL.md for full details.
...
7. **Group.get() vs Group.getFirst() differ.** `get(x, y)` is shorthand for `getFirst(false, true, x, y)` -- finds first _inactive_ member and creates if none found. `getFirst(state)` defaults to `active===false` without auto-creating.
