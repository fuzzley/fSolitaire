...
**Related skills:** ../phaser-scenes/SKILL.md, ../phaser-loading-assets/SKILL.md, ../phaser-scale-and-responsive/SKILL.md
...
Only the first scene starts automatically. Others start only if they have `{ active: true }` in their scene config. See ../phaser-scenes/SKILL.md.
...
game.events.on("hidden", () => {
  /* Page Visibility API: page hidden */
});
game.events.on("visible", () => {
  /* Page Visibility API: page visible */
});
game.events.on("pause", () => {
  /* game.pause() was called */
});
game.events.on("resume", () => {
  /* game.resume() was called */
});
```
