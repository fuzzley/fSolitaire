...
2. **Viewport vs Scroll confusion.** `setPosition` / `setViewport` move the camera's rendering rectangle on screen. `setScroll` / `scrollX` / `scrollY` move what the camera is _looking at_ in the world. These are independent.
